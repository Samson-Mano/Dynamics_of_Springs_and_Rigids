...
void modal_elementline_list_store::set_buffer(int selected_mode)
{
	...
	for (auto& line_m : modal_elementlineMap)
	{
		...
		for (auto& h_lines : ln.discretized_bar_line_data)
		{
			...
			glm::vec3 pt1_contour_color = geom_parameters::getContourColor_d(static_cast<float>(1.0 - pt1_displ_scale));
			...
			glm::vec3 pt2_contour_color = geom_parameters::getContourColor_d(static_cast<float>(1.0 - pt2_displ_scale));
