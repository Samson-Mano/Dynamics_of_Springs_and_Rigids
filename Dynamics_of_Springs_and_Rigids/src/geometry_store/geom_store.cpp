	...
	element_prop_window* elm_prop_window)
{
	...
	modal_elim_s.clear_results(); 
	pulse_elim_s.clear_results();
	freq_elim_s.clear_results();

	modal_penalty_s.clear_results();
	pulse_penalty_s.clear_results();
	freq_penalty_s.clear_results();

	modal_lagrange_s.clear_results();
...
void geom_store::paint_model()
{
		...
		forcedresp_solver_window->is_show_window == true)
	{
		...
		if (pulse_solver_window->is_show_window == true && this->is_pulse_analysis_complete == true &&
...
void geom_store::paint_modal_analysis_results()
{
	...
	if (modal_solver_window->execute_modal_close == true)
	{
		...
		if (this->is_modal_analysis_complete == true)
		{
	...
	if (modal_solver_window->execute_modal_open == true)
	{
		...
		if (this->is_modal_analysis_complete == true)
		{
			...
			modal_solver_window->mode_result_str = modal_elim_s.mode_result_str;
	...
	if (modal_solver_window->execute_modal_analysis == true)
	{
		...
		this->solver_type = modal_solver_window->solver_type;

		if (this->solver_type == 0)
		{
			// Penalty method


			modal_penalty_s.modal_analysis_penaltymethod_start(model_nodes,
				model_lineelements,
				node_constraints,
				node_ptmass,
				...
				this->is_modal_analysis_complete);
			...
			{
				// update the modal window list box
				modal_solver_window->mode_result_str = modal_penalty_s.mode_result_str;
		...
		else if (this->solver_type == 1)
		{
			// Lagrange method

			modal_lagrange_s.modal_analysis_lagrangemethod_start(model_nodes,
				model_lineelements,
				node_constraints,
				node_ptmass,
				elm_prop_window->material_list,
			...
			{
				// update the modal window list box
				modal_solver_window->mode_result_str = modal_lagrange_s.mode_result_str;
		...
		else if (this->solver_type == 2)
		{
			// Elimination method


			modal_elim_s.modal_analysis_elimmethod_start(model_nodes,
				model_lineelements,
				node_constraints,
				node_ptmass,
				elm_prop_window->material_list,
...
void geom_store::paint_pulse_analysis_results()
{
	...
	{
		// Execute the close sequence
		if (this->is_pulse_analysis_complete == true)
		{
			// Pulse response analysis is complete
			update_model_transperency(false);
		}
	...
	{
		// Execute the open sequence
		if (this->is_modal_analysis_complete == false)
		{
		...
		else
		{
			...
			if (this->solver_type == 0)
			{
				...
				pulse_solver_window->number_of_modes = static_cast<int>(modal_penalty_s.m_eigenvalues.size());
				pulse_solver_window->modal_first_frequency = std::sqrt(modal_penalty_s.m_eigenvalues.at(0)) / (2.0 * m_pi); // std::sqrt(modal_results.eigen_values[i]) / (2.0 * m_pi);
				pulse_solver_window->modal_end_frequency = std::sqrt(modal_penalty_s.m_eigenvalues.at(pulse_solver_window->number_of_modes - 1)) / (2.0 * m_pi);
				pulse_solver_window->mode_result_str = modal_penalty_s.mode_result_str;
				...
				if (this->is_pulse_analysis_complete == true)
				{
					...
					pulse_solver_window->time_interval_atrun = pulse_penalty_s.time_interval;
					pulse_solver_window->time_step_count = pulse_penalty_s.time_step_count;
			...
			else if (this->solver_type == 2)
			{
				...
				pulse_solver_window->number_of_modes = static_cast<int>(modal_elim_s.m_eigenvalues.size());
				pulse_solver_window->modal_first_frequency = std::sqrt(modal_elim_s.m_eigenvalues.at(0)) / (2.0 * m_pi); // std::sqrt(modal_results.eigen_values[i]) / (2.0 * m_pi);
				pulse_solver_window->modal_end_frequency = std::sqrt(modal_elim_s.m_eigenvalues.at(pulse_solver_window->number_of_modes - 1)) / (2.0 * m_pi);
				pulse_solver_window->mode_result_str = modal_elim_s.mode_result_str;

				// Modal analysis is complete (check whether frequency response analysis is complete or not)
				if (this->is_pulse_analysis_complete == true)
				{
					// Set the pulse response analysis result
					pulse_solver_window->time_interval_atrun = pulse_elim_s.time_interval;
					pulse_solver_window->time_step_count = pulse_elim_s.time_step_count;
	...
	if (pulse_solver_window->execute_pulse_analysis == true)
	{
		...
		if (this->solver_type == 0)
		{
			...
			pulse_penalty_s.pulse_analysis_penaltymethod_start(model_nodes,
				model_lineelements,
				node_constraints,
				node_loads,
				node_ptmass,
				...
				this->is_pulse_analysis_complete);
		...
		else if (this->solver_type == 2)
		{
			...
			pulse_elim_s.pulse_analysis_elimmethod_start(model_nodes,
				model_lineelements,
				node_constraints,
				node_loads,
				node_ptmass,
	...
	if (forcedresp_solver_window->execute_forcedresp_open == true)
	{
		...
		else
		{
			...
			if (this->solver_type == 0)
			{
				...
				forcedresp_solver_window->number_of_modes = static_cast<int>(modal_penalty_s.m_eigenvalues.size());
				forcedresp_solver_window->modal_first_frequency = std::sqrt(modal_penalty_s.m_eigenvalues.at(0)) / (2.0 * m_pi); // std::sqrt(modal_results.eigen_values[i]) / (2.0 * m_pi);
				forcedresp_solver_window->modal_end_frequency = std::sqrt(modal_penalty_s.m_eigenvalues.at(forcedresp_solver_window->number_of_modes - 1)) / (2.0 * m_pi);
				forcedresp_solver_window->mode_result_str = modal_penalty_s.mode_result_str;
			...
			else if (this->solver_type == 2)
			{
				...
				forcedresp_solver_window->number_of_modes = static_cast<int>(modal_elim_s.m_eigenvalues.size());
				forcedresp_solver_window->modal_first_frequency = std::sqrt(modal_elim_s.m_eigenvalues.at(0)) / (2.0 * m_pi); // std::sqrt(modal_results.eigen_values[i]) / (2.0 * m_pi);
				forcedresp_solver_window->modal_end_frequency = std::sqrt(modal_elim_s.m_eigenvalues.at(forcedresp_solver_window->number_of_modes - 1)) / (2.0 * m_pi);
				forcedresp_solver_window->mode_result_str = modal_elim_s.mode_result_str;
			}
	...
	if (forcedresp_solver_window->execute_forcedresp_analysis == true)
	{
		if (forcedresp_solver_window->is_selected_count == true)
		{
			...
			if (this->solver_type == 0)
			{
				...
				freq_penalty_s.forcedresp_analysis_penaltymethod_start(forcedresp_solver_window->frf_data,
					...
					this->is_forcedresp_analysis_complete);
			...
			else if (this->solver_type == 2)
			{
				...
				freq_elim_s.forcedresp_analysis_elimmethod_start(forcedresp_solver_window->frf_data,
					forcedresp_solver_window->frf_chart_setting,
					model_nodes,
					model_lineelements,
					node_constraints,
			...
			forcedresp_solver_window->forcedresp_analysis_complete = this->is_forcedresp_analysis_complete;
